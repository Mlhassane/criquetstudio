...
//     case 4:
//       return "grid-cols-2 md:grid-cols-4 max-w-6xl";
//     case 5:
//     case 6:
...
//     default:
//       return "grid-cols-2 md:grid-cols-3 lg:grid-cols-4 max-w-7xl"; // default layout
//   }
// };
...
//   return (
//     <main className="p-6 mx-auto fade-in">
...
//             Plongez dans nos différentes catégories pour découvrir les derniers
//             articles.
//           </p>
...
//             Nos équipes travaillent à ajouter du contenu.
//           </p>
//         </div>
//       ) : (
//         <div className={`grid gap-6 md:gap-8 ${getGridLayout(categories.length)} mx-auto`}>
...
//             >
//               <div className="absolute inset-0 bg-gradient-to-r from-red-500 to-rose-500 transform -translate-y-full group-hover:translate-y-0 opacity-0 group-hover:opacity-5 transition-all duration-700"></div>
//               <div className="flex items-center gap-3 mb-5 relative">
//                 <div className="p-3 rounded-xl bg-red-50 text-red-600 group-hover:bg-red-100 group-hover:text-red-700 transition-colors duration-300">
...
//                   </span>
//                   <span className="flex items-center text-sm font-medium text-red-600 group-hover:translate-x-1 transition-transform duration-300">
//                     Explorer
//                     <svg
...
async function CategoriesContent() {
  ...
  return (
    <main className="p-6 max-w-7xl mx-auto fade-in">
      ...
      {categories.length === 0 ? (
        <div className="text-center py-20 bg-secondary rounded-xl text-gray-500 shadow-inner">
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {categories.map((cat, index) => (
            <Link
              href={`/rubriques/${cat.slug?.current}`}
              key={cat._id}
              className="group block p-8 rounded-2xl bg-primary/2 shadow-md hover:shadow-2xl transition-all duration-500 category-card overflow-hidden relative"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-primary/2 to-secondary transform -translate-y-full group-hover:translate-y-0 opacity-0 group-hover:opacity-5 transition-all duration-700"></div>
              <div className="flex items-center gap-3 mb-5 relative">
                <div className="p-3 rounded-xl bg-primary text-secondary group-hover:bg-primary/10 group-hover:text-primary transition-colors duration-300">
                  <BadgeCheck className="h-7 w-7 group-hover:scale-110 transition-transform" />
                </div>
                <h2 className="text-2xl font-bold text-primary group-hover:text-primary/80 transition-colors duration-300">
                  {cat.title}
                </h2>
              </div>
              ...
              <div className="pt-4 border-t border-gray-100">
                <div className="flex items-center justify-between">
                  ...
                  <span className="flex items-center text-sm font-medium text-primary group-hover:translate-x-1 transition-transform duration-300">
                    Explorer
                    <svg
                      className="w-5 h-5 ml-1"
                      fill="none"
